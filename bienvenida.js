document.addEventListener("DOMContentLoaded", () => {
  ...
  tabs.forEach(tab=>{
    tab.addEventListener("click", ()=> {
      ...
      localStorage.setItem("tabActiva", tab.dataset.tab);
    });
  });
  ...
  if(tabGuardada) {
    const boton = document.querySelector(`[data-tab="${tabGuardada}"]`);
  ...
  const mostrarTareas = () => {
    ...
    tareas.forEach((t,i)=>{
      ...
      li.addEventListener("click", ()=>{
        ...
        guardarTareas();
        // reproducir sonido, con control de existencia
        try { sonido.play(); } catch(e){}
        mostrarTareas();
      });
